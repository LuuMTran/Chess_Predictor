...
async function main() {
  setLoading(true);
  try {
    let confidentInput = document.getElementById('confident').value;
    let confident = confidentInput ? 1 - parseFloat(confidentInput) : 0.5;

    let num_game_input = document.getElementById('num_game').value;
    let num_game = num_game_input ? parseInt(num_game_input) : 10; // Default to 10 games if empty
    ...
    // Check if undefined, which might happen on error
    if (number_win_needed === undefined) {
        document.getElementById('result').textContent = "Prediction failed. Please check your inputs.";
        return;
    }

    let rest_game = num_game - number_win_needed

    // Display result in the result container instead of alert
    let resultText = `Bet Suggestion: (Friend - You): ${rest_game} - ${number_win_needed}`;
